...
class helpAction extends frontendAction {
	...
	function getMenu(){
		$arrMenu = array(
				'about' => array('text'=>'关于我们', 'url'=>U('public/help/about')),
				'contact' => array('text'=>'联系我们', 'url'=>U('public/help/contact')),
				'agreement' => array('text'=>'用户条款', 'url'=>U('public/help/agreement')),
				'privacy' => array('text'=>'隐私声明', 'url'=>U('public/help/privacy')),
				'download' => array('text'=>'源码下载', 'url'=>U('public/help/download')),
		);
		return $arrMenu;
	}
	public function download(){
		...
		if(empty($from)){
			...
			if($from == 5)
			{
				...
				if(!false == $this->down_mod->create($data)){
					$this->down_mod->add();
					header('Location: http://www.ikphp.com/down/IKPHP1.5.3.rar');
				}
			}			
