...
class adminAction extends backendAction {
	...
	public function article_list($nameid){
		...
		!$strChannel && $this->error ( '没有该频道的内容；或许你还没有创建频道!', U('article/admin/channel'));
		// 获取分类
	...
	public function article_isaudit($nameid){
		...
		if($strItem){
			...
			$this->redirect ( 'article/admin/index',array('ik'=>'list','nameid'=>$nameid,'isaudit'=>$isaudit));
		}else{
	...
	public function article_istop($nameid){
		...
		if($strItem){
			...
			$this->redirect ( 'article/admin/index',array('ik'=>'list','nameid'=>$nameid,'isaudit'=>$strItem['isaudit']));
		}else{
			$this->error('文章不存在或已被删除！');
		}
	}	
	...
	public function channel() {
		$ik = $this->_get ( 'ik', 'trim' ,'list');
		$menu = array(
				'list' => array('text'=>'频道设置', 'url'=>U('article/admin/channel',array('ik'=>'list'))),
				'add' => array('text'=>'新建频道', 'url'=>U('article/admin/channel',array('ik'=>'add'))),
		);
	...
	public function channel_nav(){
		...
		if(!empty($nameid)){
			...
			$this->redirect ( 'article/admin/channel',array('ik'=>'list'));
		}
	...
	public function cate_add($nameid){
		if(IS_POST){
			...
			if(empty($catename)) $this->error('添加失败，分类名称不能为空');
			if(empty($nameid)) $this->error('你还没有创建频道吧；先创建频道再新建分类吧！',U('article/admin/channel'));
			if(!empty($nameid) && !empty($catename)){
				//执行添加
				if(!false == $this->cate_mod->create(array('catename'=>$catename,'nameid'=>$nameid))){
					$this->cate_mod->add();
					$this->redirect ( 'article/admin/cate',array('ik'=>'list','nameid'=>$nameid));
				}
			}else{
	...
	public function cate_edit(){
		...
		if($strCate){
			if(IS_POST){
				...
				$this->redirect ( 'article/admin/cate',array('ik'=>'list','nameid'=>$strCate['nameid']));
			}else{
	...
	public function cate_delete(){
		...
		if($strCate){
			if(IS_POST){
				...
				$this->redirect ( 'article/admin/cate',array('ik'=>'list','nameid'=>$newCate['nameid']));
			}else{
