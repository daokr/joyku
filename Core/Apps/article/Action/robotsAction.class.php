...
class robotsAction extends backendAction {
	...
	public function lists() {
		// 获取采集列表
		$list = $this->robots_mod->order ( 'addtime desc' )->select ();
	...
	public function publish() {
		...
		if ($robotid > 0) {
			...
			$this->robots_mod->where ( $where )->save ( $data );
			$this->robots_mod->updaterobot ( $robotid ); // 更新采集器缓存
			$this->success ( "采集机器人编辑成功", U ( 'article/robots/lists' ) );
		
		} else {
			
			if (! false == $this->robots_mod->create ( $data )) {
				$robotid = $this->robots_mod->add ();
			}
			$this->robots_mod->updaterobot ( $robotid ); // 更新采集器缓存
			$this->success ( "采集机器人成功添加", U ( 'article/robots/lists' ) );
		}
	...
	public function edit() {
		...
		$thevalue = $this->robots_mod->where ( array (
				'robotid' => $robotid 
		) )->find ();
		// 获取全部频道
	...
	public function delete() {
		...
		$res = $this->robots_mod->where ( $map )->find ();
		if (! empty ( $res )) {
			$this->robots_mod->where ( $map )->delete ();
					...
					if ($mpage >= count ( $newurlarr )){
						...
						if (! empty ( $_GET ['pageurl'] )){
							...
							if (! empty ( $msgmsgarr ['message'] ))
								$itemid = $this->robots_mod->messageaddtodb ( $msgmsgarr, $_GET ['robotid'], $itemid );
						...
						if (! empty ( $messagetext )){
							...
							if (empty ( $status )) {
								...
								$this->robots_mod->where(array('robotid'=>$robotid))->setField($map);
								$status = 1;
							}
							...
							{  
								// 插入到库中
								$itemid = $this->robots_mod->messageaddtodb ( $msgarr, $robotid, 0 );
								if($itemid>0){
