...
<div class="mc">
	<h1><?php echo ($seo["title"]); ?>(<?php echo ($strUser[count_followed]); ?>)</h1>
	<div class="cleft">

<ul class="user-list">
<?php if(is_array($arrFollowedUser)): foreach($arrFollowedUser as $key=>$item): ?><li class="clearfix" id="u$item[userid]">
        <a title="<?php echo ($item[username]); ?>" href="<?php echo U('space/index/index',array('id'=>$item[doname]));?>">
        <img alt="<?php echo ($item[username]); ?>" src="<?php echo ($item[face]); ?>" class="face">
        </a>
        <div class="info">
          <h3>
              <a title="<?php echo ($item[username]); ?>" href="<?php echo U('space/index/index',array('id'=>$item[doname]));?>"><?php echo ($item[username]); ?></a>
              <?php if($item[isfollow] == 1): ?><span class="user-cs">已关注</span><?php endif; ?>
          </h3>
          <!-- 签名档 -->
...
<?php if($strUser[userid] == $visitor[userid]): ?><p class="pl2">
            &gt;&nbsp;<a href="<?php echo U('public/user/follow',array('userid'=>$strUser[userid]));?>">我关注的人(<?php echo ($strUser[count_follow]); ?>)</a>
        </p> 
<?php else: ?>
        <p class="pl2">
            &gt;&nbsp;<a href="<?php echo U('public/user/follow',array('userid'=>$strUser[userid]));?>"><?php echo ($strUser[username]); ?>关注的人(<?php echo ($strUser[count_follow]); ?>)</a>
        </p><?php endif; ?>              
<!---
        <p class="pl2">
...
-->        
        <p class="pl2">
            &gt;&nbsp;<a href="<?php echo U('public/user/contacts',array('d'=>'invite'));?>">邀请我的朋友加入爱客网</a>
        </p>
