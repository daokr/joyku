...
#pop_win_login { width:300px; padding: 10px }
#pop_win_login form { width: 300px; border: none }
#pop_win_login fieldset { border: 0 none; padding: 0; margin: 0 }
#pop_win_login .item { margin: 15px 0 12px 0; overflow: visible }
#pop_win_login label { display: inline-block; float:left; margin-right: 15px; text-align: right; width: 40px; font-size: 14px; line-height: 30px; vertical-align: baseline }
...
<h3>会员登录<span> ( <a href="<?php echo U('public/user/register');?>" >还没有注册?</a> ) </span></h3>

<div id="pop_win_login" class="">
    <form class="pop_win_login_form" name="lzform" method="post" action="<?php echo U('public/user/login');?>">
        <fieldset>
            <input type="hidden" name="ret_url" value="<?php echo ($jump); ?>" />
            <input type="hidden" name="cktime" value="2592000">
            <div class="item">
                <label for="form_email">帐号</label>
                <input tabindex="1" type="text" id="form_email" name="email" class="text pop_email" value="邮箱"/>
            </div>
            <div class="item">
                <label for="form_password">密码</label>
                <input tabindex="2" type="password" name="password" id="pop_password" class="text"/>
            </div>
            <div class="item remember">
                <label>&nbsp;</label>
                <input id="remember" type="checkbox" name="remember" tabindex="4"/>
                <label for="remember" class="sub">下次自动登录 |</label><a href="<?php echo U('public/user/forgetpwd');?>" style="margin-left: 0.8em;" >忘记密码了</a>
            </div>
