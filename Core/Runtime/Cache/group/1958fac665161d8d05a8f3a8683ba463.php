...
<?php if($page==1){ ?>
      ...
      <div class="topic-content clearfix">
        <div class="user-face"> <a href="<?php echo U('space/index/index',array('id'=>$strTopic[user][doname]));?>"><img title="<?php echo ($strTopic[user][username]); ?>" alt="<?php echo ($strTopic[user][username]); ?>" src="<?php echo avatar($strTopic['userid'], 48);?>" width="48"></a></div>
        <div class="topic-doc">
          <h3> <span class="color-green"><?php echo date('Y-m-d H:i:s',$strTopic[addtime]) ?></span> <span class="pl20">来自：<a href="<?php echo U('space/index/index',array('id'=>$strTopic[user][doname]));?>"><?php echo ($strTopic[user][username]); ?></a></span> </h3>
          ...
          <?php if(($user[userid] == $strTopic[userid]) OR ($user[userid] == $strGroup[userid]) OR ($strGroupUser[isadmin] == 1) OR ($user[isadmin] == 1) ): ?><div style="text-align:right;"> 
                <?php if(($user[userid] == $strGroup[userid]) OR ($strGroupUser[isadmin] == 1) OR ($user[isadmin] == 1)): ?>&gt;&nbsp; 
                    <a href="<?php echo U('group/index/topic',array('d'=>'topic_istop','topicid'=>$strTopic[topicid]));?>"><?php echo ($action[istop]); ?></a> &gt;&nbsp; 
                    <a href="<?php echo U('group/index/topic',array('d'=>'isdigest','topicid'=>$strTopic[topicid]));?>"><?php echo ($action[isdigest]); ?></a> &gt;&nbsp; 
                    <a href="<?php echo U('group/index/topic',array('d'=>'isshow','topicid'=>$strTopic[topicid]));?>"><?php echo ($action[isshow]); ?></a>  
                    <!--&gt;&nbsp;<a href="<?php echo U('group/index/topic',array('d'=>'topic_move','groupid'=>$strTopic[groupid],'topicid'=>$strTopic[topicid]));?>"><?php echo ($action[move]); ?></a>--><?php endif; ?>
            	&gt;&nbsp; 
            	<a href="<?php echo U('group/index/topic',array('d'=>'topic_edit','topicid'=>$strTopic[topicid]));?>">编辑</a> &gt;&nbsp; 
            	<a href="<?php echo U('group/index/topic',array('d'=>'deltopic','topicid'=>$strTopic[topicid]));?>" onclick="return confirm('确定删除?')">删除</a> 
      ...
      <?php if($visitor['userid']): ?><div class="sns-bar">
        	<div class="sns-bar-rec">
                <span class="rec">
                    <a class="bn-sharing  i a_share_btn" data-pic="<?php echo ($strTopic[content_photo][0]); ?>" data-title="<?php echo ($strTopic[title]); ?>" data-desc="<?php echo getsubstrutf8(t($strTopic[content]),0,150) ?>" data-url="<?php echo U('group/index/topic',array('id'=>$strTopic[topicid]));?>" href="#">分享到</a> &nbsp;&nbsp;
                </span>            
            	<div class="rec-sec"><a href="<?php echo U('group/index/topic',array('d'=>'topic_recommend'));?>" title="推荐" class="lnk-sharing i a_recommend_btn"  data-title="<?php echo getsubstrutf8(t($strTopic[title]),0,40); ?>"  data-desc="<?php echo getsubstrutf8(t($strTopic[content]),0,100) ?>"  data-tkind="<?php echo ($strTopic[groupid]); ?>" data-tid="<?php echo ($strTopic[topicid]); ?>" data-tuid="<?php echo ($IK_USER['user']['userid']); ?>" data-url="<?php echo U('group/index/topic',array('id'=>$strTopic[topicid]));?>">推荐</a> <span class="rec-num" id="rec-num"><?php echo ($strTopic[count_recommend]); ?>人</span></div>
            </div>
            <div class="sns-bar-fav">
            	<span  class="fav-num"><a href="javascript:;" id="like-num"><?php echo ($strTopic[count_collect]); ?>人</a> 喜欢 </span>
            	<?php if($strTopic[islike]): ?><a href="<?php echo U('group/index/topic',array('d'=>'topic_collect'));?>" title="取消喜欢" data-tkind="<?php echo ($strTopic[groupid]); ?>" data-tid="<?php echo ($strTopic[topicid]); ?>" data-tuid="<?php echo ($user['userid']); ?>" class="btn-fav fav-cancel i a_like_btn">喜欢</a> 
				<?php else: ?>
                <a href="<?php echo U('group/index/topic',array('d'=>'topic_collect'));?>" title="标为喜欢" data-tkind="<?php echo ($strTopic[groupid]); ?>" data-tid="<?php echo ($strTopic[topicid]); ?>" data-tuid="<?php echo ($user['userid']); ?>" class="btn-fav fav-add i a_like_btn">喜欢</a><?php endif; ?>
            </div>        
        </div><?php endif; ?>
      ...
      <div class="tags"> 
        <?php if(is_array($strTopic[tags])): $i = 0; $__LIST__ = $strTopic[tags];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><a rel="tag" title="<?php echo ($item[tagname]); ?>" class="post-tag" href="<?php echo U('group/index/explore_topic',array('tag'=>$item[tagname]));?>"><?php echo ($item[tagname]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
...
<?php } ?>
      ...
      <div class="clear"></div>
      <div> 
     	 <?php if(!empty($upTopic)): ?>上一篇：<a href="<?php echo U('group/index/topic',array('id'=>$upTopic['topicid']));?>"><?php echo ($upTopic['title']); ?></a><?php endif; ?>&nbsp;&nbsp;&nbsp;&nbsp;
         <?php if(!empty($downTopic)): ?>下一篇：<a href="<?php echo U('group/index/topic',array('id'=>$downTopic['topicid']));?>"><?php echo ($downTopic['title']); ?></a><?php endif; ?>
      </div>
    ...
    <div class="cright">
       <?php if($isGroupUser): ?><div class="side-reg" id="g-side-info-member">
          <div class="bd">
              <div class="group-item">
                  <div class="pic">
                       <a href="<?php echo U('group/index/show',array('id'=>$strGroup[groupid]));?>" title="<?php echo ($strGroup[groupname]); ?>"><img src="<?php echo ($strGroup[icon_48]); ?>" alt="<?php echo ($strGroup[groupname]); ?>"></a>
                  </div>
                  <div class="info">
                      <div class="title">
                          <a href="<?php echo U('group/index/show',array('id'=>$strGroup[groupid]));?>" title="<?php echo ($strGroup[groupname]); ?>"><?php echo getsubstrutf8(t($strGroup[groupname]),0,14) ?></a>
                      </div>
     ...
     <?php else: ?>
      <div class="side-reg" id="g-side-info">
        <div class="bd">
          <div class="group-item">
            <div class="pic"> <a href="<?php echo U('group/index/show',array('id'=>$strGroup[groupid]));?>"> <img src="<?php echo ($strGroup[icon_48]); ?>"> </a> </div>
            <div class="info">
              <div class="title"> <a href="<?php echo U('group/index/show',array('id'=>$strGroup[groupid]));?>"><?php echo getsubstrutf8(t($strGroup[groupname]),0,14) ?></a> </div>
        ...
        <div class="ft">
          <div class="member-status"><a class="bn-join" href="<?php echo U('group/index/join', array('id'=>$strGroup['groupid']));?>">加入小组</a></div>
        </div>
      </div><?php endif; ?>
...
<br clear="all">
      ...
      <div class="indent newtopic">
        <ul>
          <?php if(is_array($newTopic)): $i = 0; $__LIST__ = $newTopic;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><li><a title="<?php echo ($item[title]); ?>" href="<?php echo U('group/index/topic', array('id'=>$item[topicid]));?>"><?php echo ($item[title]); ?></a> &nbsp; <span class="pl">(<a href="<?php echo U('space/index/index',array('id'=>$item[user][doname]));?>"><?php echo ($item[user][username]); ?></a>) </span> </li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ul>
      </div>
      <div class="clear"></div>
